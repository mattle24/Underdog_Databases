    ...
    <div id = 'page-header1'>
        <div class="spacer"></div>
          ...
          <?php
            ...
            $term = FALSE; // where there any search terms? init as FALSE
            if (isset($_POST['donorid'])) {
                $searchid = filter_input(INPUT_POST,'donorid',FILTER_SANITIZE_STRING);
                $term = TRUE;
            } else {$searchid = FALSE;}
            ...
            if (isset($_POST['searchzip'])) {
                $searchzip = filter_input(INPUT_POST, 'searchzip', FILTER_SANITIZE_STRING);
                $searchzip = '%'.trim($searchzip).'%'; // for wildcards
                $term = TRUE;
            } else {$searchzip = '%';}
           ...
           if (!$term) {
               echo '<p>You have not entered search details. Please go back and try again.</p>';
                ...
                $cmp = $_SESSION['cmp'];
                $donor_table = $cmp."_donors";
                if ($searchid) {
                    $query = "SELECT donorid, First_Name, Last_Name, Street_Number, Street_Name, City
                    FROM $donor_table
                    WHERE voter_id = ?
                    AND First_Name LIKE ?
                    AND Last_Name LIKE ?
